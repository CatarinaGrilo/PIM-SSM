...
class PacketPimHelloDrPriority(PacketPimHelloOptions):
    PIM_HDR_OPT = "! L"
    PIM_HDR_OPT_LEN = struct.calcsize(PIM_HDR_OPT)
    '''
     0                   1                   2                   3
    ...
    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    '''
    def __init__(self, DR_priority: int):
        super().__init__(type=19, length=4)
        self.DR_priority = DR_priority

    def bytes(self) -> bytes:
        return super().bytes() + struct.pack(self.PIM_HDR_OPT, self.DR_priority)

    @staticmethod
    def parse_bytes(data: bytes, type:int = None, length:int = None):
        if type is None or length is None:
            raise Exception
        (DR_priority, ) = struct.unpack(PacketPimHelloDrPriority.PIM_HDR_OPT,
                                     data[:PacketPimHelloDrPriority.PIM_HDR_OPT_LEN])
        #print("GenerationID:", generation_id)
        return PacketPimHelloDrPriority(DR_priority=DR_priority)
