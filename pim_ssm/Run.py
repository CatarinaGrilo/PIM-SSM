    ...
    def run(self):
        ...
        while True:
            try:
                ...
                elif 'dr_priority' in args and args.dr_priority:
                    Main.dr_priority(args.dr_priority[0])
                elif 'add_interface' in args and args.add_interface:
                    Main.add_pim_interface(args.add_interface[0])
...
def main():
    """
    Entry point for PIM-SSM
    """
    parser = argparse.ArgumentParser(description='PIM-SSM', prog='pim-ssm')
    ...
    group.add_argument("-dr", "--dr_priority", nargs=1, metavar='DR_PRIORITY', help="Change DR priority")
    ...
    if os.geteuid() != 0:
        sys.exit('PIM-SSM must be run as root!')
    ...
    if args.start:
        ...
        os.system("tail -f /var/log/pimssm/stdout")
        sys.exit(0)
